...
```
AWS_ACCUIaws sts gtcaller-dentit --e cot --ot te)
s-as-
APIEPO_NAME=capstone-eventtsapiWeRA=capstone-eventweb
EVENTSJOB_REPO_NAME=capstone-eventsjob
```
### 2.2 Create Repositories
```
aws ecr create-repository --repository-name $API_REPO_NAME
aws ecr create-repository --repository-name $_REPO_NAME
...
![image](https://github.com/user-attachmentsWeb_REPO_NAME/assets/40731d69-c7ef-4c05-bf6b-05cd7df3da46)
...
### 2. Creating and Storing API Container on EC Registry

Run this from the events-api directory

```
cd ~/aws-minicapstone-week2-Kuberneoer-attentsa1
...
```.uand Te the ories the esite me estry
...
or ntwebsite
```
docker run -d -p 8080:8080 -e SERVER='http://localhost:8082' --network="host" $Web_ECR_URI:
```
estourab e a wer o your ian
...
```
aws ecr create-repository --repository-name $API_REPO_NAME
aws ecr create-repository --repository-name $# Use aws public image
...
aws ecr create-repository --repository-name $EVENTSJOBWeb_REPO_NAME
```
From the ECR console verify these new three registrees are created
...
### 2.6**13.4  Run and Test the docker images the Website from the Registr Locally**
...
![image](https://github.com/user-attachments/assets/94445e86-4804-4e64e26e2193-1ef7-4371-a1a0-dcbce3064360)
...
Now you completed theYou should be able to creatione a new version of your application using Docker commands.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIyMjYwNjE4XX0=
-->